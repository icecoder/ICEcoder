...
// ==============
	...
	renderLineStyle:	false,		// False or contains the style to apply
...
// ==============
	...
	cMonRenderLine: function(thisCM,cMinstance,line,element) {
		// If we have a style to use when rendering the line
		if (top.ICEcoder.renderLineStyle) {
			// If the line we're rendering is the line we have a style set for, set it
			if (thisCM.lineInfo(line).line+1 == top.ICEcoder.renderLineStyle[1]) {
				element.style[top.ICEcoder.renderLineStyle[2]] = top.ICEcoder.renderLineStyle[3];
			}
		}
	},

	updateDiffs: function() {
		var cM, cMdiff, mainText, diffText, sm, opcodes, cMmarks, cMdiffMarks, amt, markColor, lineNum;
		...
		// Clear any styles set for render line event and set our main vs diff pane shift difference to 0
		top.ICEcoder.renderLineStyle = false;
		...
		for (var i=0; i<opcodes.length; i++) {
			...
			if (opcodes[i][0] !== "equal") {
				...
				if (opcodes[i][0] == "replace") {
					...
					// Min of defaultTextHeight() as a failsafe
					if (amt <= 0) {amt = cM.defaultTextHeight();};
					// Set the style to use when we render the line in just a moment
					top.ICEcoder.renderLineStyle = ["main", opcodes[i][2], "height", amt + "px"];
				}
				// Replacing is grey, green in this pane if inserting
				markColor = opcodes[i][0] == "replace" ? "diffGrey" : "diffGreen";
				// Mark the range with that class
				cM.markText({line: opcodes[i][1], ch: 0}, {line: opcodes[i][2]-1, ch: 1000000}, {className: markColor});
				// Revert to no line rendering styles again
				top.ICEcoder.renderLineStyle = false;
				...
				if (opcodes[i][0] != "replace" && opcodes[i][3] == opcodes[i][4]) {
					top.ICEcoder.renderLineStyle = ["diff", opcodes[i][4], "height", ((opcodes[i][2] - opcodes[i][1] + 1) * cM.defaultTextHeight()) + "px"];
				...
				if (opcodes[i][0] == "replace") {
					...
					if (amt <= 0) {amt = cM.defaultTextHeight();};
					// Set the style to use when we render the line in just a moment
					top.ICEcoder.renderLineStyle = ["diff", opcodes[i][4], "height", amt + "px"];
				}
				// Replacing is grey, red in this pane if deleting
				markColor = opcodes[i][0] == "replace" ? "diffGrey" : "diffRed";
				// Mark the range with that class
				cMdiff.markText({line: opcodes[i][3], ch: 0}, {line: opcodes[i][4]-1, ch: 1000000}, {className: markColor});
				// Revert to no line rendering styles again
				top.ICEcoder.renderLineStyle = false;
				...
				if (opcodes[i][0] != "replace" && opcodes[i][1] == opcodes[i][2]) {
					top.ICEcoder.renderLineStyle = ["main", opcodes[i][2], "height", ((opcodes[i][4] - opcodes[i][3] + 1) * cM.defaultTextHeight()) + "px"];
					// Mark the range with empty class
					cM.markText({line: opcodes[i][2]-1, ch: 0}, {line: opcodes[i][2]-1, ch: 1000000}, {className: "diffNone"});
					// Revert to no line rendering styles again
					top.ICEcoder.renderLineStyle = false;
				}
				// Set the last amount shifted for this change
				top.ICEcoder.renderPaneShiftAmount = (opcodes[i][2] - opcodes[i][4]);
			}
		}
		// Finally, revert to no line rendering styles again
		top.ICEcoder.renderLineStyle = false;	
	},
	...
	openPreviewWindow: function() {
		if (top.ICEcoder.openFiles.length>0) {
			...
			if (["md"].indexOf(fileExt) > -1) {
				top.ICEcoder.previewWindow.onload = function() {top.ICEcoder.previewWindow.document.documentElement.innerHTML = mmd(thisCM.getValue())};
			} else {

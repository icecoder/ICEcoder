...
// ==============
	...
	updateFileManagerList: function(action,location,file,perms,oldName,uploaded,fileOrFolder) {
		...
		if (action=="add" && !get('filesFrame').contentWindow.document.getElementById(location.replace(iceRoot,"").replace(/\/$/, "").replace(/\//g,"|")+"|"+file)) {
			...
			innerLI = '<a nohref title="'+location.replace(/\/$/, "")+"/"+file+'" onMouseOver="parentNode.draggable=true;this.overFileFolder(\''+actionElemType+'\',this.childNodes[1].id)" onMouseOut="parentNode.draggable=false;this.overFileFolder(\''+actionElemType+'\',\'\')" '+
					...
					(actionElemType == "folder" ? 'this.openCloseDir(this,'+(actionElemType=="folder" ? 'true' : 'false')+');':'')+
...
// ==============
	...
	codeAssistToggle: function() {
		...
		get('codeAssistDisplay').style.backgroundPosition = this.codeAssist ? "0 0" : "-16px 0";
		this.cssColorPreview();

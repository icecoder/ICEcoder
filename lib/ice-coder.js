		...
		if (evt.altKey) {
			...
			// Enter (Expand dir/open file)
				...
				if(!evt.ctrlKey && !top.ICEcoder.cmdKey) {
					var selElem = top.get('filesFrame').contentWindow.document.getElementById(top.ICEcoder.selectedFiles[top.ICEcoder.selectedFiles.length-1]+"_perms").parentNode;
					if (selElem.onmouseover.toString().indexOf("'folder'") > -1) {
						top.ICEcoder.openCloseDir(selElem,true);
					} else {
						top.ICEcoder.openFile(selElem.childNodes[1].id.replace(/\|/g,"/"));
					}
					return false;
				}
			...
			// Up/down/left/right arrows (Traverse files)
				...
				if(!evt.ctrlKey && !top.ICEcoder.cmdKey) {
					var selElem = top.get('filesFrame').contentWindow.document.getElementById(top.ICEcoder.selectedFiles[top.ICEcoder.selectedFiles.length-1]+"_perms").parentNode;
					var fileFolder = selElem.onmouseover.toString().indexOf("'folder'") > -1 ? "folder" : "file";
					var goElem = false;
					// Up on this depth
					if(key==38) {
						if (selElem.parentNode.previousSibling && selElem.parentNode.previousSibling.previousSibling) {
							var goElem = selElem.parentNode.previousSibling.previousSibling;
							if (goElem.tagName == "UL") {goElem = goElem.previousSibling} // Jump over UL DOM elem
							goElem = goElem.childNodes[0];
						}
					// Down on this depth
					} else if(key==40) {
						if (selElem.parentNode.nextSibling && selElem.parentNode.nextSibling.nextSibling) {
							var goElem = selElem.parentNode.nextSibling.nextSibling;
							if (!goElem.tagName) {goElem = goElem.nextSibling} // Jump over a non DOM elem
							goElem = goElem.childNodes[0];
						}
					// Left, go up a dir
					} else if(key==37) {
						if (selElem.parentNode.parentNode.previousSibling) {
							var goElem = selElem.parentNode.parentNode.previousSibling.childNodes[0];
						}
					// Right, jump to first item in this dir
					} else if(key==39) {
						if (selElem.parentNode.nextSibling && selElem.parentNode.nextSibling.childNodes[0]) {
							var goElem = selElem.parentNode.nextSibling.childNodes[0].childNodes[0];
					...
					if (goElem) {
						top.ICEcoder.overFileFolder(fileFolder, goElem.childNodes[1].id);
						top.ICEcoder.selectFileFolder(evt);
					}
				}
	        		return false;

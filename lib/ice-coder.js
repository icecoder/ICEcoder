...
// ==============
	...
	highlightGitDiffs: function() {
		...
		if ("undefined" != typeof top.highlightGitDiffTimeout) {
			clearTimeout(top.highlightGitDiffTimeout);
		}
		// If we have index data & Git data, after a timeout, if we have a matching path in that Git data
		if (top.ICEcoder.indexData && top.ICEcoder.indexData.gitContent) {
			top.highlightGitDiffTimeout = setTimeout(function() {
				if (top.ICEcoder.indexData.gitContent[top.docRoot+top.ICEcoder.openFiles[top.ICEcoder.selectedTab-1]]) {
					...
					var gitText = difflib.stringAsLines(top.ICEcoder.indexData.gitContent[top.docRoot+top.ICEcoder.openFiles[top.ICEcoder.selectedTab-1]].lastHashContent);
			...
			},top.ICEcoder.loadingFile ? 100 : 0);
		}
	},
	...
	updateGitDiffPane: function() {
		var gitDiffList = "";
		for (var i=0; i<top.ICEcoder.indexData.gitDiff.paths.length; i++) {
			gitDiffList +=
			'<div class="link" onclick="top.ICEcoder.toolShowHideToggle(\'git\'); top.ICEcoder.openFile(\'/' +
			top.ICEcoder.indexData.gitDiff.paths[i] +
			"')\">" +
			top.ICEcoder.indexData.gitDiff.paths[i] +
			"</div>" +
			"\n";
			top.get("git").innerHTML = gitDiffList + "<br><br>";
		}
	},
		...
		if (top.ICEcoder.copiedFiles) {
			for (var i=0; i<top.ICEcoder.copiedFiles.length; i++) {
				if (top.ICEcoder.copiedFiles[i]!="|") {
					top.ICEcoder.serverQueue("add","lib/file-control-xhr.php?action=paste&location="+location+"&csrf="+top.ICEcoder.csrf,encodeURIComponent(top.ICEcoder.copiedFiles[i]));

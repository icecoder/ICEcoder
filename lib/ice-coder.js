...
// ==============
	...
	cMonChange: function(thisCM,cMinstance,changeObj) {
		...
		if (top.ICEcoder.findMode) {
			...
			top.document.getElementById('results').innerHTML = top.ICEcoder.results.length + " " + top.t['results'];
	...
	updateDiffs: function() {
		var cM, cMdiff, mainText, diffText, sm, opcodes, cMmarks, markColor;
		...
		for (var i=0; i<opcodes.length; i++) {
			// We have a 'replace' or 'delete' status
			if (opcodes[i][0] !== "equal") {
				markColor = opcodes[i][0] == "replace" ? "diffGrey" : "diffGreen";
				cM.markText({line: opcodes[i][1], ch: 0}, {line: opcodes[i][2]-1, ch: 1000000}, {className: markColor, startStyle: markColor, endStyle: markColor});
				markColor = opcodes[i][0] == "replace" ? "diffGrey" : "diffRed";
				cMdiff.markText({line: opcodes[i][3], ch: 0}, {line: opcodes[i][4]-1, ch: 1000000}, {className: markColor, startStyle: markColor, endStyle: markColor});
			}
		}
	},

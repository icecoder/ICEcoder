...
d-1;for(f=1;f<c;f++)top.get("saveTemp"+f).value=top.get("saveTemp"+(f+1)).value;c=top.get("saveTemp"+c);c.parentNode.removeChild(c)}ICEcoder.serverQueueItems.splice(0,1)}if("del"==a&&1<=ICEcoder.serverQueueItems.length||1==ICEcoder.serverQueueItems.length)if(b&&0<b.indexOf("action=save")){saveURL=ICEcoder.serverQueueItems[0];var e=top.ICEcoder.xhrObj();e.onreadystatechange=function(){if(4==e.readyState&&200==e.status){var a=JSON.parse(e.responseText);a.status.error?top.ICEcoder.message(a.status.errorMsg):
eval(a.action.doNext)}};e.open("POST",saveURL,!0);e.setRequestHeader("Content-type","application/x-www-form-urlencoded");e.send("contents="+top.document.getElementById("saveTemp1").value)}else setTimeout(function(){top.ICEcoder.filesFrame.contentWindow.frames.fileControl.location.href=ICEcoder.serverQueueItems[0]},1)},cancelAllActions:function(){window.stop();0<ICEcoder.serverQueueItems.length&&ICEcoder.serverQueueItems.splice(1,ICEcoder.serverQueueItems.length);top.ICEcoder.showHide("hide",top.get("loadingMask"));

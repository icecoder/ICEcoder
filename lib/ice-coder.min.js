...
d-1;for(f=1;f<c;f++)top.get("saveTemp"+f).value=top.get("saveTemp"+(f+1)).value;c=top.get("saveTemp"+c);c.parentNode.removeChild(c)}ICEcoder.serverQueueItems.splice(0,1)}if("del"==a&&1<=ICEcoder.serverQueueItems.length||1==ICEcoder.serverQueueItems.length)b&&0<b.indexOf("action=save")?(e=top.ICEcoder.xhrObj(),e.onreadystatechange=function(){4==e.readyState&&200==e.status&&(g=JSON.parse(e.responseText),g.action.timeEnd=(new Date).getTime(),g.action.timeTaken=g.action.timeEnd-g.action.timeStart,g.status.error?
top.ICEcoder.message(g.status.errorMsg):eval(g.action.doNext))},e.open("POST",ICEcoder.serverQueueItems[0],!0),e.setRequestHeader("Content-type","application/x-www-form-urlencoded"),c=(new Date).getTime(),e.send("timeStart="+c+"&contents="+encodeURIComponent(top.document.getElementById("saveTemp1").value))):setTimeout(function(){top.ICEcoder.filesFrame.contentWindow.frames.fileControl.location.href=ICEcoder.serverQueueItems[0]},1)},cancelAllActions:function(){window.stop();0<ICEcoder.serverQueueItems.length&&

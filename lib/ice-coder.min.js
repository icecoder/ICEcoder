...
b=difflib.stringAsLines(b);b=(new difflib.SequenceMatcher(b,a)).get_opcodes();for(var c=0;c<b.length;c++)for(j=b[c][3];j<b[c][4];j++)"equal"!=b[c][0]&&("undefined"==typeof b[c][5]&&(b[c][5]=""),b[c][5]+=a[j]+"\n");return JSON.stringify(b)},saveFile:function(a){var b,c,d,e;a||(b=top.ICEcoder.getChangesToSave());c=top.ICEcoder.getcMInstance().getValue().length;a=a?"saveAs":"save";d=ICEcoder.openFiles[ICEcoder.selectedTab-1].replace(top.iceRoot,"").replace(/\//g,"|");"|[NEW]"==d&&0<top.ICEcoder.selectedFiles.length&&
(e=top.ICEcoder.selectedFiles[0],d=-1==e.lastIndexOf(".")||e.lastIndexOf(".")<e.lastIndexOf("|")?e+d:"|[NEW]");d=d.replace("||","|");top.ICEcoder.serverQueue("add","lib/file-control-xhr.php?action=save&fileMDT="+ICEcoder.openFileMDTs[ICEcoder.selectedTab-1]+"&fileVersion="+ICEcoder.openFileVersions[ICEcoder.selectedTab-1]+"&saveType="+a+"&charCount="+c+"&csrf="+top.ICEcoder.csrf,d.replace(/\+/g,"%2B"),b);top.ICEcoder.serverMessage("<b>"+top.t.Saving+"</b><br>"+ICEcoder.openFiles[ICEcoder.selectedTab-
